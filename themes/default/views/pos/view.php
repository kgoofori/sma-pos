            ...
            <table class="table table-striped table-condensed">
              <thead>
                ...
                foreach ($rows as $row) {
                    ...
                    echo '<tr><td>#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name) .'</td>';
                    if ($row->item_discount != 0) {
                    echo '<td>' . $this->sma->formatMoney($row->net_unit_price + ($row->item_discount / $row->quantity) + ($row->item_tax / $row->quantity)) . '</td> ';
                    }
                    else
                    {
                    echo '<td>' . $this->sma->formatNumber($row->net_unit_price). '</td>';
                    }
                    echo '<td>' . number_format($row->quantity). '</td> ';

                    echo '<td>' . $this->sma->formatNumber($row->quantity * $row->net_unit_price). '</td></tr> ';
